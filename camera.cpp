...
void CAMERA::calcColor()
{
    Mat imgHSV;								//用于保存hsv图像
    //Mat imgOriginal = src.getMat();			//原图像
    vector<thold> vecThold;					//存储阈值的容器向量
    vector<Mat> hsvSplit;					//分别储存hsv三通道向量容器

    vecThold.push_back(thold(Scalar(156, 43, 46), Scalar(180, 255, 255), Scalar(0, 0, 255), R"(Red)"));//红色
    vecThold.push_back(thold(Scalar(35, 43, 46), Scalar(77, 255, 255), Scalar(0, 255, 0), R"(Green)"));//绿色
    vecThold.push_back(thold(Scalar(100, 43, 46), Scalar(124, 255, 255), Scalar(255, 0, 0), R"(Blue)"));//蓝色
    vecThold.push_back(thold(Scalar(26, 43, 46), Scalar(34, 255, 255), Scalar(0, 255, 255), R"(Yellow)"));//黄色
    ...
    for (const auto& vp : vecThold)
    {
        ...
        for (const auto& cp : contours)										//遍历
        {
            ...
            if (rect.area() > 25000 && rect.area() < 40000)										//方框面积大于30000设别出来
            {
